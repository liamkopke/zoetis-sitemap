	...
	async function generateHTML(endpointObject) {
        ...
        */
			...
			if (
				key !== "/" &&
				key !== "/fr" &&
				(endpointMasterEn["/"] == endpointObject ||
					endpointMasterFr["/fr"] == endpointObject) &&
				Object.keys(endpointObject[key]).length === 0
			) {
...
function getHref(key, endpoints) {
	const x = endpoints.filter((data) => {
		return data.endpoint.endsWith(key);
	});

	if (x === []) {
		x = endpoints.filter((data) => {
			return data.endpoint.endsWith(key + "/");
		});
	}
